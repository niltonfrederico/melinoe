...
2. Craft a `system_prompt` that reflects the soul's persona and purpose based on the description
3. Choose an appropriate `ModelConfig` preset (conversational souls often benefit from CLAUDE_SONNET for quality)
4. Create `melinoe/workflows/souls/{name}.py` with a complete Soul class (name, system_prompt, model_config, history, chat, reset)
5. Update `melinoe/workflows/souls/__init__.py` to export the new class
