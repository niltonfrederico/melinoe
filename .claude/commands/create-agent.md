...
2. Infer which existing Skills (if any) the agent should compose, or leave `steps` as an empty list with a comment for what to add
3. Create `melinoe/workflows/agents/{name}.py` with a complete `Workflow` subclass
4. Update `melinoe/workflows/agents/__init__.py` to export the new class
